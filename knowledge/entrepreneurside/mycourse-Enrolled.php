...
include "../Utils/Validation.php";
   ...
   if (isset($_GET['course_id'])) {
      ...
      $course_id = Validation::clean($_GET['course_id']);
        ...
        Util::redirect("allcourses.php", "error", $em);
    }
    ...
    $loggedin_userid = $_SESSION['id_user'];
    $data = array($course_id, $loggedin_userid);
    $res = check_enrolled_student($data);

    if ($res == 0) {
      $em = "Invalid course id ";
      // Util::redirect("allcourses.php", "course_id", $course_id);
    }
    $progress = getStudentProgress($course_id, $loggedin_userid);
    if ($progress >= 100) {
      $progress = 100;
    }
      ...
      <ul class="list-group" id="sideMenu">
        <?php $i=0; foreach($course['chapters'] as $chapter) { $i++; 
            if ($chapter['chapter_id'] == $_chapter_id) {
                ...
                if ($cureent >= $progress) {
                 updateStudentProgress($course_id, $loggedin_userid, $progress_plus);
                }
            }
          ...
          <li class="list-group-item">
            <a href="javascript:void()" class="btn badge-primary">
             ...
             <ul>
              ...
              ?>
               <li>
                 <a href="mycourse-Enrolled.php?course_id=<?=$course_id?>&chapter_id=<?=$topic['chapter_id']?>&topic_id=<?=$topic['topic_id']?>" class="btn badge-primary" style="color: #0D6EFD;" >
                  <b><?=$topic['title']?></b></a>
               </li>
               <?php }else { ?>
                <li>
                 <a href="mycourse-Enrolled.php?course_id=<?=$course_id?>&chapter_id=<?=$topic['chapter_id']?>&topic_id=<?=$topic['topic_id']?>" class="btn badge-primary"><?=$topic['title']?></a>
               </li>
               <?php } } } ?>
             </ul>
           </li>
