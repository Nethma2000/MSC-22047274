...
<div class="container">
    ...
    <div class="list-table pt-5">
        ...
        <h4>All Courses Materials (<?=$row_count?>) <a href="addcoursematerials.php" class="btn btn-primary">Upload Material</a></h4><br>
        ...
        <table class="table table-bordered">
            ...
            <tr>
                <td><a href="<?=$CoursesMaterial["URL"]?>" target="_blank"><img src="assets/img/Logo.jpg" width="100"></a></td>
                <td><?=$CoursesMaterial["type"]?></td>
                <td class="status"><mark><code class="p-2"><?=$CoursesMaterial["URL"]?></code></mark></td>
                <td><a href="<?=$CoursesMaterial["URL"]?>" class="btn btn-primary" download>Download</a></td>
            </tr>
            <?php } ?>
        </table>
        <?php if ($last_page > 1 ) { ?>
        <div class="d-flex justify-content-center mt-3 border">
            ...
            if ($prev_btn){
            ?>
            <a href="coursematerials.php?page=<?=$prev?>" class="btn btn-secondary m-2">Prev</a>
            <?php } else { ?>
            ...
            for($i = $push_mid; $i < 5 + $page; $i++){
                if($i == $page){ ?>
                <a href="coursematerials.php?page=<?=$i?>" class="btn btn-success m-2"><?=$i?></a>
                <?php } else { ?>
                <a href="coursematerials.php?page=<?=$i?>" class="btn btn-secondary m-2"><?=$i?></a>
                <?php } 
                if($last_page <= $i) break;
            }
            if($next_btn){
            ?>
            <a href="coursematerials.php?page=<?=$next?>" class="btn btn-secondary m-2">Next</a>
            <?php } else { ?>
